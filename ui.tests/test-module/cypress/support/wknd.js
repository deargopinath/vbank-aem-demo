...
// VBANKCreateContentPage creates a random content page, the page details can be accessed with this.vbankPage
// NOTE: Don't use an arrow function as they don't have their own bindings to this, arguments, or super,

Cypress.Commands.add('VBANKCreateContentPage', function (dir, title) {
  if (!title) {
  ...
  cy.get('.foundation-collection-item[data-foundation-collection-item-id="/conf/vbank/settings/wcm/templates/content-page-template"]').click()
  // click on next
  ...
  cy.wrap(
    { path: dir + '/' + title, title }).as('vbankPage')
})
