 ...
 */
package com.demo.web.digital.vbank.it.tests;
...
public class GraphQlIT {
    ...
    private static final String VBANK_SHARED_GRAPHQL_ENDPOINT = "/content/_cq_graphql/vbank-shared/endpoint.json";
    ...
    private static AEMHeadlessClient getHeadlessClient(InstanceConfiguration instanceConfig) throws URISyntaxException {
        final String graphQLEndpoint = instanceConfig.getUrl() + VBANK_SHARED_GRAPHQL_ENDPOINT;
    ...
    public void testPersistedQuery() {
        GraphQlResponse response = headlessClientAuthor.runPersistedQuery("/vbank-shared/adventures-all");
        assertNull(response.getErrors());
        JsonNode responseData = response.getData();
    ...
    public void testListPersistedQueries() {
        List<PersistedQuery> listPersistedQueries = headlessClientAuthor.listPersistedQueries("vbank-shared");

        assertFalse(listPersistedQueries.isEmpty());
        PersistedQuery adventuresQuery = listPersistedQueries.stream()
                .filter(p -> p.getShortPath().equals("/vbank-shared/adventures-all")).findFirst().get();
        assertEquals("/vbank-shared/settings/graphql/persistentQueries/adventures-all", adventuresQuery.getLongPath());
